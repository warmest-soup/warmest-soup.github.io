...
/*Function Calls*/ {
  /*Initial exp goal Assignments*/ {
    ...
    expGoalCalc(perReal, perExp, perGoal);
    expGoalCalc(chaReal, chaExp, chaGoal);
    expGoalCalc(navReal, navExp, navGoal);
    expGoalCalc(endReal, endExp, endGoal);
    expGoalCalc(conReal, conExp, conGoal);
    expGoalCalc(staReal, staExp, staGoal);
    expGoalCalc(intReal, intExp, intGoal);
    expGoalCalc(wisReal, wisExp, wisGoal);
    expGoalCalc(knoReal, knoExp, knoGoal);
    //weight goal calc
    weightGoalCalc(wgtReal, wgtExp, wgtGoal);
  ...
  /*Initial assignments of Func trait values*/ {
    ...
    catBonus(perReal, aweBonus, perBonus, perFunc);
    catBonus(chaReal, aweBonus, chaBonus, chaFunc);
    catBonus(navReal, aweBonus, navBonus, navFunc);
    catBonus(endReal, bodBonus, endBonus, endFunc);
    catBonus(conReal, bodBonus, conBonus, conFunc);
    catBonus(staReal, bodBonus, staBonus, staFunc);
    catBonus(intReal, mndBonus, intBonus, intFunc);
    catBonus(wisReal, mndBonus, wisBonus, wisFunc);
    catBonus(knoReal, mndBonus, knoBonus, knoFunc);
  ...
  /*Initial assignment of Exp Decay Factors*/ {
    var secSklMem = Math.min(spdFunc.innerText, dexFunc.innerText);
    var secInsMem = Math.min(intFunc.innerText, wisFunc.innerText);
  }
  ...
  /*event listener fot the Real Traits*/ {
    ...
    dexReal.addEventListener("input", function () {
      expGoalCalc(dexReal, dexExp, dexGoal);
      catBonus(dexReal, athBonus, dexBonus, dexFunc);
      levelCalc(TraitRanks, Ranks);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
    ...
    spdReal.addEventListener("input", function () {
      expGoalCalc(spdReal, spdExp, spdGoal);
      catBonus(spdReal, athBonus, spdBonus, spdFunc);
      levelCalc(TraitRanks, Ranks);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
    ...
    perReal.addEventListener("input", function () {
      expGoalCalc(perReal, perExp, perGoal);
      catBonus(perReal, aweBonus, perBonus, perFunc);
      levelCalc(TraitRanks, Ranks);
    });
    chaReal.addEventListener("input", function () {
      expGoalCalc(chaReal, chaExp, chaGoal);
      catBonus(chaReal, aweBonus, chaBonus, chaFunc);
      levelCalc(TraitRanks, Ranks);
    });
    navReal.addEventListener("input", function () {
      expGoalCalc(navReal, navExp, navGoal);
      catBonus(navReal, aweBonus, navBonus, navFunc);
      levelCalc(TraitRanks, Ranks);
    });
    endReal.addEventListener("input", function () {
      expGoalCalc(endReal, endExp, endGoal);
      catBonus(endReal, bodBonus, endBonus, endFunc);
      levelCalc(TraitRanks, Ranks);
    });
    conReal.addEventListener("input", function () {
      expGoalCalc(conReal, conExp, conGoal);
      catBonus(conReal, bodBonus, conBonus, conFunc);
      levelCalc(TraitRanks, Ranks);
      thermoBarFunc(worldTemp, TempBar, TempFill);

      satBarFill(document.getElementById("shBar"), Hunger);
      satBarFill(document.getElementById("swBar"), Water);
      satBarFill(document.getElementById("szBar"), Sleep);
      satBarFill(document.getElementById("stBar"), Toxic[0]);
    });
    staReal.addEventListener("input", function () {
      expGoalCalc(staReal, staExp, staGoal);
      catBonus(staReal, bodBonus, staBonus, staFunc);
      wildCalc(staFunc, intFunc);
      levelCalc(TraitRanks, Ranks);
    });
    intReal.addEventListener("input", function () {
      expGoalCalc(intReal, intExp, intGoal);
      catBonus(intReal, mndBonus, intBonus, intFunc);
      levelCalc(TraitRanks, Ranks);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
      secInsMem = Math.min(intFunc.innerText, wisFunc.innerText);
    });
    wisReal.addEventListener("input", function () {
      expGoalCalc(wisReal, wisExp, wisGoal);
      catBonus(wisReal, mndBonus, wisBonus, wisFunc);
      levelCalc(TraitRanks, Ranks);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
    ...
    knoReal.addEventListener("input", function () {
      expGoalCalc(knoReal, knoExp, knoGoal);
      catBonus(knoReal, mndBonus, knoBonus, knoFunc);
      levelCalc(TraitRanks, Ranks);
      thermoBarFunc(knoReal, TempBar, TempFill);
    });
  ...
  /*Event Listeners for category bonueses*/ {
    athBonus.addEventListener("input", function () {
      catBonus(strReal, athBonus, strBonus, strFunc);
      catBonus(dexReal, athBonus, dexBonus, dexFunc);
      catBonus(spdReal, athBonus, spdBonus, spdFunc);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
    ...
    aweBonus.addEventListener("input", function () {
      catBonus(perReal, aweBonus, perBonus, perFunc);
      catBonus(chaReal, aweBonus, chaBonus, chaFunc);
      catBonus(navReal, aweBonus, navBonus, navFunc);
    });
    bodBonus.addEventListener("input", function () {
      catBonus(endReal, bodBonus, endBonus, endFunc);
      catBonus(conReal, bodBonus, conBonus, conFunc);
      catBonus(staReal, bodBonus, staBonus, staFunc);
      thermoBarFunc(worldTemp, TempBar, TempFill);
    ...
    mndBonus.addEventListener("input", function () {
      catBonus(intReal, mndBonus, intBonus, intFunc);
      catBonus(wisReal, mndBonus, wisBonus, wisFunc);
      catBonus(knoReal, mndBonus, knoBonus, knoFunc);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
  ...
  /*Event Listener for Trait Bonuses*/ {
    strBonus.addEventListener("input", function () {
      catBonus(strReal, athBonus, strBonus, strFunc);
      carryWeight();
    });
    dexBonus.addEventListener("input", function () {
      catBonus(dexReal, athBonus, dexBonus, dexFunc);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
      secSklMem = Math.min(spdFunc.innerText, dexFunc.innerText);
    });
    spdBonus.addEventListener("input", function () {
      catBonus(spdReal, athBonus, spdBonus, spdFunc);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
      secSklMem = Math.min(spdFunc.innerText, dexFunc.innerText);
    });
    perBonus.addEventListener("input", function () {
      catBonus(perReal, aweBonus, perBonus, perFunc);
    });
    chaBonus.addEventListener("input", function () {
      catBonus(chaReal, aweBonus, chaBonus, chaFunc);
    });
    navBonus.addEventListener("input", function () {
      catBonus(navReal, aweBonus, navBonus, navFunc);
    });
    endBonus.addEventListener("input", function () {
      catBonus(endReal, bodBonus, endBonus, endFunc);
    });
    conBonus.addEventListener("input", function () {
      catBonus(conReal, bodBonus, conBonus, conFunc);
      satBarFill(
        document.getElementById("swBar"),
        document.getElementById("WaterVal")
    ...
    staBonus.addEventListener("input", function () {
      catBonus(staReal, bodBonus, staBonus, staFunc);
    });
    intBonus.addEventListener("input", function () {
      catBonus(intReal, mndBonus, intBonus, intFunc);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
      secInsMem = Math.min(intFunc.innerText, wisFunc.innerText);
    });
    wisBonus.addEventListener("input", function () {
      catBonus(wisReal, mndBonus, wisBonus, wisFunc);
      slotMemoryLimits(Slots, spdFunc, dexFunc, intFunc, wisFunc);
      secInsMem = Math.min(intFunc.innerText, wisFunc.innerText);
    });
    knoBonus.addEventListener("input", function () {
      catBonus(knoReal, mndBonus, knoBonus, knoFunc);
    });
  ...
  /*Event Listeners for Sat Values.*/ {
    Hunger.addEventListener("change", function () {
      satBarFill(document.getElementById("shBar"), Hunger);
      weightExpGain(
        document.getElementById("wgtReal"),
        document.getElementById("wgtExp"),
